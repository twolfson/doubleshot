...
Doubleshot.prototype = _.defaults({
  ...
  loadFiles: function (fn) {
      ...
      var valObj = valMapper.lookup(key, {
        ...
        middlewares: [function resolveObjectAliases (val) {
          ...
          if (typeof val === 'object') {
            ...
            var obj = val;
            ['before', 'beforeEach', 'afterEach', 'after'].forEach(function resolveBeforeEtcAliases (key) {
              // If the key exists
              var val = obj[key];
              if (val !== undefined) {
                // Resolve its value
                // TODO: I think we need to do some re-arranging to allow for middlewares resolving middlewares?
                // TODO: This might also be the proper injection point for an 'if it is an object, throw an error. you can't alias to an object bro.'
                // TODO: ^ Definitely test the cannot alias a `Object.after` to an `Object.after`, it must be a 'string' alias.
                content.__doubleshot_tmp = val;
                var valObj = this.lookup('__doubleshot_tmp', {alias: true, map: true, flatten: true});
    ...
    // TODO: Attach after chaining tests
    // TODO: Contemplated introspecting object and doing lookups via that mechanism but it is not practical (crosses over with value-mapper logic)
