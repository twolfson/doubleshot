        ...
        middlewares: [function resolveObjectAliases (val) {
          ...
          if (typeof val === 'object') {
            ...
            ['before', 'beforeEach', 'afterEach', 'after'].forEach(function resolveBeforeEtcAliases (hookName) {
              ...
              if (hooks !== undefined) {
                // Resolve their value
                // DEV: We don't need our custom middleware here since resolving the value of an object will be rejected
                // TODO: If we do object detection here, we will get a semantic message who is to/from
                ...
                // TODO: NEED TO POST UPDATE FOR VALUE-MAPPER FOR THIS.KEY USAGE
                var _val = valObj.value;
                if (typeof _val === 'object') {
                  throw new Error('Aliasing an object from another object is not allowed. Please update "' + this.key + '.' + hookName + '"');
                }

                // Save the value
                retVal[hookName] = valObj.value;

                // Save the key for error reference
                lastObjKey = this.key;
              }
            }, this);
          }
